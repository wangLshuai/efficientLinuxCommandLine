#/bin/bash
PROGRAM=$(basename $0)
DATABASE=`pwd`/vault.gpg

load_password() {
	# Place username (field 1) into clipboard
	echo "$1" | tr -s ' '|cut -d ' ' -f1 |tr -d '\n' |xclip -selection clipboard
	# Place password (feld 2) into X primary selection
	echo "$1" | tr -s ' ' |cut -d ' ' -f2 |tr -d '\n' | xclip -selection primary
	# Give feedback to the user
	echo "$PROGRAM: Found" $(echo "$1" | cut -f3- --output-delimiter ': ')
	echo "$PROGRAM: username and password loaded into X selectons"
}
...
# Stroe the decrypted text in a variable
